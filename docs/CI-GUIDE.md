...
| ビルド後（毎回） | **ユニットテスト結果（TRX / サマリ / 失敗ログ）をプロジェクト配下の `tests/` へ保存（releases/logs と同じ入れ子）** |
...
> **.NET の Release 成果物について**: publish は **framework-dependent 単一ファイル**（`*-win-x64.exe`、`.NET` ランタイム非同梱・アプリ依存は exe に取り込み）です。**実行 PC 側に対応する .NET ランタイムがインストールされている前提**の運用です（self-contained にするとランタイム込みで肥大化するため採用していません）。後方互換で同内容の `*-win-x64.zip` も出力します。`publishProject` に指定する csproj は実行アプリ（`OutputType` が `Exe`/`WinExe`）である必要があります。ライブラリ（`IpuTestAppCore` 等）が指定されている場合、CI は実行アプリ csproj を探して差し替えます。見つからなければ NETSDK1099 の前に失敗します。
...
└── MyApp\                          # 成果物・ログ・解析・ソース・テスト（すべてプロジェクト配下）
    ...
    └── tests\20260615\MyApp-42-103000\    # ユニットテスト結果（毎回保存・他カテゴリと同じ入れ子）
        ├── test-results.trx
        ├── test-summary.json
        └── test-failures.log      # 失敗があれば
```

> **開発環境一式の zip 保存（任意）:** ③ の「開発環境 zip（source）」行のチェックを ON にすると、CI が **チェックアウト済みのソースツリー**を毎回 zip 化し、各書き込み先の `source\[日付]\<プレフィックス>-<ビルド番号>-src.zip` へ格納します。zip からは `.git` / `artifacts` / `bin` / `obj` / `.vs` / `node_modules` / `packages` / `TestResults` / `*.user` を除外します。フォルダ名は ③「開発環境 zip（source）— 保存フォルダ名」(`storage.sourceDir`、既定 `source`) で変更できます。Teams から開く URL は ④ の同カテゴリ欄（`storage.sourceUrls`）へ貼り付けます。
...
> **開発環境一式の zip 保存（任意）:** ③「開発環境 zip（source）」行のチェックを ON にすると、CI が毎回チェックアウト済みソースを zip 化して各書き込み先の `source\[日付]\<プレフィックス>-<ビルド番号>-src.zip` に保存します。`.git` / `artifacts` / `bin` / `obj` / `.vs` / `node_modules` / `packages` / `TestResults` / `*.user` は除外。フォルダ名は ③ の保存フォルダ名欄（`storage.sourceDir`）で変更可。閲覧用 URL は ④ の同カテゴリ欄（`storage.sourceUrls`）へ。
...
| **閲覧 URL**（④） | Teams から開く**共有リンク（URL）** | `https://contoso.sharepoint.com/:f:/s/share/xxxx` |

③ と ④ は同じカテゴリ表示名で対応します（左が保存フォルダ、右が閲覧 URL）。
...
※ source は `archiveSource` が ON のときのみ有効。

- Jenkins エージェントで OneDrive クライアントが対象ライブラリを同期している必要があります。ローカル同期フォルダへ書けば OneDrive がクラウドへ同期します。
- 書き込み先（パス欄）に共有 URL を入れると保存時にエラー（または CI で配置スキップ）になります。URL は ④ Teams の各 URL 欄へ入れてください。
- 書き込み先・各 URL 欄ともに、パス/URL を自動判別して結合（`\` / `/`）します。

> **書き込み先は「複数」設定できます（重要）:** ③「成果物・ログの保存先」の **共有フォルダルート（`CI_FILE_SERVER`）** と **書き込み先ベース（`storage.basePaths`）** は **併用でき**、各欄の右端「＋」で行を増やせます（「−」で削除）。デプロイ時は **設定した全ての書き込み先へコピー** します（相互排他ではありません）。
...
>
> **閲覧用 URL も複数設定できます:** ④ Teams の各カテゴリ URL 欄（logs / releases / analysis / tests / source）も「＋」で複数指定でき、Teams 通知では全リンクを（2 件以上なら連番付きで）ボタン表示します。source の URL 欄は `archiveSource` が ON かつ source フォルダ作成済みのときだけ入力可能です。
...
2. （任意）SharePoint 等の共有 URL を ③ と同じカテゴリ名の各欄（logs / releases / analysis / tests / source）に入力
3. **「テスト送信」** をクリック
4. **ゲート:** Teams にテストカードが届く（設定した URL に対応するボタン付き）
...
2. 進行: 最新の取り込み（git pull）→ 保存 → ローカルでビルド＆テスト → Jenkins 反映 → テストビルド
